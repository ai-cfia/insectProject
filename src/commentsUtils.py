...
from dates import get_yesterday

from src.apiUtils import createlinktofullimageVer2
from src.preprocessUtils import getLocation_perObservation
...
def getComments(maxCountTrials, dates, queryPairs, commentStatus, iconic_taxa):
    ...
    s2 = 0
    countTrials = 0
    while (not queryIsComplete) and (
        countTrials < maxCountTrials
    ):  # api will throttle and the code will crash
        i, j, allComments, commentStatus = runQuery(
            s1, s2, dates, queryPairs, allComments, commentStatus, iconic_taxa
        )
        s1, s2 = i, j
...
def runQuery(s1, s2, dates, queryPairs, allComments, commentStatus, iconic_taxa):
    for i in range(s1, len(dates)):
        ...
        for j in range(s1, len(boundingboxes)):
            ...
            try:
                allComments, commentStatus = runQuery_Datei_Boxj(
                    allComments, commentStatus, iconic_taxa, box, date
                )
            except:
...
def runQuery_Datei_Boxj(allComments, commentStatus, iconic_taxa, box, date):
    ...
    newComments, commentStatus = getNewComments(
...
def getNewComments(commentStatus, iconic_taxa, datefrom, dateto, loc):
    page = str(1)
    observations_page_k, totalNumberOfObservations = getObservationsC(
        iconic_taxa, datefrom, dateto, page, str(200), loc
    )
    comments, ids, commentStatus = getComments_page_k(
        observations_page_k, commentStatus
    )
    ...
    for page_number in range(2, numberOfPages + 1):
        ...
        observations_page_k, _ = getObservationsC(
            iconic_taxa, datefrom, dateto, page, str(200), loc
        )
        comments_k, ids_k, commentStatus = getComments_page_k(
            observations_page_k, commentStatus
        )
...
def getComments_page_k(observations_page_k, commentStatus):
    ...
    for observation in observations_page_k:
        ...
        if (
            numberOfComments
            == commentStatus[
                commentStatus["observation_id"] == observation_id
        ...
        comments_of_observation, commentStatus = getComments_observation(
            observation, observation_id, numberOfComments, newObservation, commentStatus
        )

        if (
            len(comments_of_observation) != 0
        ):  # only add comments of interest (i.e., in Canada): country check done inside
...
def getComments_observation(
    ...
    if not newObservation:
        if (
            ...
            != ""
        ):  # if country is not CA
            return [], commentStatus

        # get new comments only
        previousCommentCount = commentStatus[
            commentStatus["observation_id"] == observation_id
        ].comment_count.values
        ...
        commentStatus.loc[
            commentStatus.observation_id == observation_id, "comment_count"
        ] = numberOfComments
