...
ordered=['Quality', 'Name1', 'Name2','City', 'Province', 'Obsvn_URL', 'Sample_Img', 'posted_on',
       'observed_on', 'user_login', 'Obsvn_ID', 'upper taxa',
       'ai_visionVerification']
ordered_us=['Quality', 'Name1', 'Name2','City', 'Province', 'Obsvn_URL', 'Sample_Img', 'posted_on',
       'observed_on', 'user_login', 'Obsvn_ID', 'upper taxa']
page=1
...
if test==1: # remove
    regulated_species=[["Monochamus scutellatus","Insecta"],["Helix lucorum","Plantae"],["Anoplophora glabripennis","Insecta"],["Anoplophora chinensis","Insecta"]] # Turkish Snail, asian longhorned and citrus longhorned beetle
...
invasiveObservations=getObsv_allPages(regulated_ids,datefrom,dateto,geobound,page,columns_in_out)
invasiveObservations_us=getObsv_allPages(regulated_ids_us,datefrom,dateto,geobound_us,page,columns_in_out)
...
invasiveObservations=invasiveObservations.drop(invasiveObservations[nonIvasiveRows].index).reset_index(drop=True )
...
for model in models: 
    for specie in model["nonInvasive"]:
        if test==1: # remove (so many observations of white spotted so we need to remove them for the ip address not to be banned)
            datefrom="2020-10-01"
            dateto="2020-10-15"
        specie_id=[specie['id']]
        nonInvasiveLookAlikes_i=getObsv_allPages(specie_id,datefrom,dateto,geobound,page,columns_in_out)
        ...
        nonInvasiveLookAlikes_i=nonInvasiveLookAlikes_i[nonInvasiveLookAlikes_i[MLcolumn]=='invasive'].reset_index(drop=True)
        # append invasive to output data (tobedone 1)
        #uncomment to include over the non invasive samples 
        invasiveObservations=pd.concat([invasiveObservations,nonInvasiveLookAlikes_i]) 
...
for model in models: 
    for specie in model["invasive"]:
        image_sets=invasiveObservations[invasiveObservations.Name1==specie].Sample_Img.values
        predictions=predict(image_sets,model['model'],initial)
        invasiveObservations.loc[invasiveObservations.Name1==specie,MLcolumn]=predictions    
##############################
invasiveObservations=invasiveObservations.drop_duplicates(subset=['Obsvn_ID']).reset_index(drop=True)
invasiveObservations_us=invasiveObservations_us.drop_duplicates(subset=['Obsvn_ID']).reset_index(drop=True)
##############################
...
dfInsecta, dfMollusca, dfPlantae, dfFungi, dfChromista, dfOthers = seperateByTaxa(dfRegulated,upperTaxa)
dfInsecta_us, dfMollusca_us, dfPlantae_us, dfFungi_us, dfChromista_us, dfOthers_us = seperateByTaxa(dfRegulated_us,upperTaxa)
##############################
...
for remail in receiver_email:
    print(remail)
    send_email(remail,dfInsecta, dfPlantae, dfMollusca, dfFungi, dfChromista, dfOthers, dfInsecta_us, dfMollusca_us, dfPlantae_us, dfFungi_us, dfChromista_us, dfOthers_us, Title)
